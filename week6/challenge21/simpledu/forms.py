...
from simpledu.hendlers.ws import redis, SysMsg
...
class SysMsgForm(FlaskForm):
    text = TextAreaField('消息内容', validators=[Required(), Length(3, 255)])
    submit = SubmitField('发送')

    def send(self):
        sysmsg = SysMsg()
        self.populate_obj(sysmsg)
        message = sysmsg.parse_json()
